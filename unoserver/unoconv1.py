...
class Convertor:
    def __init__(self, interface="127.0.0.1", port="2002"):
        ...
        self.context = self.resolver.resolve(
            f"uno:socket,host={interface},port={port};StarOffice.ComponentContext"
        )
    ...
    def convert11(self, inputfn):
        ...
        filtername = self.find_filter("docx", "pdf")
        output_props = (
...
def main():
    ...
    try:
        ...
        if op.stdin:
            ...
            convertor = Convertor()
            result = convertor.convert(inputfn)
        elif op.filenames:
            convertor = Convertor()
...
#        debug_office()
    ...
    class OutputStream111( unohelper.Base, XOutputStream ):
        def __init__( self ):
            self.closed = 0
